...
describe('LessonCard properties', () => {
    it('has colums', async () => {
        const el = await fixture(html `<lesson-card></lesson-card>`);
...
describe("LossonCard override attribute", () => {
    it('colums', async () => {
        const el = await fixture(html `<lesson-card colums="5"></lesson-card>`);
        expect(el.colums).to.equal(5);
    });
    it('lesson count', async () => {
        const el = await fixture(html `<lesson-card lessonsCount="9"></lesson-card>`);
        expect(el.lessonsCount).to.equal(9);
    });
    it('lesson time', async () => {
        const el = await fixture(html `<lesson-card lessonsTime="170"></lesson-card>`);
        expect(el.lessonsTime).to.equal(170);
    });
    it('rating', async () => {
        const el = await fixture(html `<lesson-card rating="3"></lesson-card>`);
        expect(el.rating).to.equal(3);
    });
    it('slot', async () => {
        const el = await fixture(html `<lesson-card >Children elem</lesson-card>`);
        expect(el.innerHTML).to.equal("Children elem");
    });
});
//# sourceMappingURL=lesson-card.test.js.map